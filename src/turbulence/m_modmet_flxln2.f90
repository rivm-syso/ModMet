...
contains
   ...
   pure function modmet_flxln2(u1, u2, zu1, zu2, T, cloud_fraction, sinphi, kin) result(result)
      ...
      max_ust = 5.0 * ust_guess ! Set a reasonable upper limit for ust
      ust_guess = 0.1*ust_guess ! Start with a smaller initial guess to ensure convergence
   ...
   contains
      ...
      pure function momentum_f(ust_g) result(fx)
         ...
         tstv = tst_out%tst + 0.00061*TR* tst_out%qst
